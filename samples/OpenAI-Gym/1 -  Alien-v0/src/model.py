...
class AlienDQN(DQNModel):
    def __init__(self, convolutional_layers: nn.Sequential, linear_layers: nn.Sequential,
        ...
        self._linear = nn.Sequential(nn.Linear(reshape_layer, linear_layers[0].in_features),
                                     linear_layers).to(
                device)
