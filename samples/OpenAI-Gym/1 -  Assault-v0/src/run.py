...
def train(env: TimeLimit, agent: BaseAgent, num_eps: int, render: bool):
    ...
    for n in range(num_eps):
        state = env.reset()
        state = cv2.resize(state, (105, 80))
        state = torch.from_numpy(state).float().unsqueeze(-1)
        ep_score = 0
        while True:
            action = agent.act(state)
            next_state, reward, done, _ = env.step(action)
            next_state = cv2.resize(next_state, (105, 80))
...
def run(seed, update_every, gamma, tau, lr, batch_size, render, gpu):
    ...
    model = AlienDQN(
            convolutional_layers=nn.Sequential(nn.Conv2d(3, 8, (3, 3)), nn.ReLU(),
                                               nn.Conv2d(8, 16, (3, 3)), nn.ReLU()),
